...
`xivl-captures`. A video breakdown is an AI-written observation document,
produced from a retail 1.x gameplay video using the prompt in
[`video-breakdown-handoff.md`](video-breakdown-handoff.md), that distills
what packets never carry: damage magnitudes, heal amounts, timing/pacing, and
visual state.

## Goal

When a new breakdown arrives, we want it to end up in a stable id, with the
raw document preserved under `sources/<id>/`, a compact evidence-map packet
under `studies/<id>/`, and a recorded evidence verdict that a consumer project
can promote on its own side with an immutable citation.

## Quick Version

1. Produce the breakdown document using the prompt in
...
- `primary_paths` - the first places an agent should open
...
skimmable. Further research records unresolved evidence work, not agent
assignments or work session plans.
...
- keep the search hints concrete enough for future agent lookup
...
1. **Dialogue:** check every quoted NPC/enemy line against the decoded client
   ...
   match (allowing for the player-name token) is hallucinated or misread - flag
