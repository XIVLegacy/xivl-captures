# Video Breakdown Handoff Prompt

Give the prompt below (everything after the horizontal rule) to a
video analysis agent together with a retail Final Fantasy XIV 1.x gameplay video
to produce an observation document we can ingest as evidence. Ingest the result
per the "Video Breakdown Intake" section of
[`integrating-new-captures.md`](integrating-new-captures.md).

---

You are extracting observations from a Final Fantasy XIV 1.x (version 1.23b, the
"Legacy" 2010-2012 client) gameplay video for a downstream consumer.
You are an evidence gatherer, not a designer. The consumer has decoded client
data dumps and a 1.23b pcap corpus covering a fixed
set of scenario families; retail 1.x video is one of the few windows into
how the live game actually behaved, and this video is the only source for
what it shows. Your output will be archived as evidence and verified
line-by-line against the decoded client data (and packets where they exist),
so a wrong specific is worse than an honest gap.

I will provide a gameplay video, the fight or content name, and optionally notes
or a previous breakdown as a format example.
...
about 1.x. The decoded 1.x client data the team holds - not the wiki - is the
...
Hard rules, in order of how often their violation has burned us:
...
Do NOT include implementation recommendations, suggested AI logic, pseudocode,
battle-controller designs, spawn-system designs, or "implementation priorities"
of any kind. The implementing side has the real engine APIs and its own design
process; invented code and architecture suggestions have proven actively
misleading. If you believe an observation has a non-obvious implementation
consequence, state the observation and one plain-language sentence about why it
...
3. **Dialogue Transcript** - every readable NPC/enemy line: exact text,
   ...
   line the team cannot get anywhere else.
...
10. **Rewards and Obtain Lines** - HIGHEST-PRIORITY quest evidence: reward
    ...
    the only anchor the team will ever get. Quote exactly: every `You obtain [item/key item]` line, gil and experience
...
13. **Uncertainties** - what the video could not establish: unread log lines,
    ...
    version) - the team needs to know if a clip might not be 1.23b-era.
