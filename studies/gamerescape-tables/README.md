...
The Gamer Escape FFXIV wiki's 1.0-era item, mob and NPC data, mined out of raw
...
defaults from, and **1,333 NPCs** with their race, clan, occupation and coordinates. Web-source evidence, **wiki tier** (packet captures > video
breakdown > wiki), so values are CALIBRATION-grade.

Check `xivl-client-data` first - item and mob names, ids, and much of the
numeric stat block are shipped by the client and are primary evidence there. Two
fields here are documented by no live FFXIV wiki, and they are why this study
exists:
...
- `derived/evidence-map.md` - tier and version caveats, the unique-value fields,
  and every dirty-data pattern (unnormalized booleans, mixed `hq_effect`
  vocabularies, undocumented `behavior_flags`). Read before the CSVs.
...
- `derived/npc-client-crosscheck-notes.md` - if you are here for NPCs. `npcs.csv` is
  the best-dated GE material in the study (263 of its 1,335 pages last edited in 2010)
  and the only source here for an NPC's race, clan, occupation and coordinates.
...
exactly such a bulk import. The corpus is maintainer-local research storage
under this source's `local-research-corpus` storage id (next to the
scraper that produced it); it is outside every repository history.
...
checks used to pin each reading. Current bytes are anchored by
...
Three more results are worth naming here because nothing else documents them.
...
- `armor` ships a **four-slot bonus run of its own and never uses it** - all four
  ...
  bonuses instead. `weapon`'s damage-type run is the same shape: slot 1 only.

## Materia

`itemData.140` turned out to be a key into the client's own `materia.csv`, which
holds the whole of 1.0 materia. It is decoded as its own artifact,
`derived/materia-decoded.csv`:

66 lines, each with four grade item ids, a primary parameter and 16 magnitudes as
4 grades x 4 ranks, an optional second parameter with 16 more, one icon per grade,
and 38 booleans for the equipment it melds into. GE's `meld_slots` names 27 of
...
spans blocks 1801 and 1803, so same-block does not mean same-group. Two more are not factions: **1620** is a
second Amalj'aa roster named by office and stronghold rather than by job, and
**1890** is the scripted-opponent block - job-quest champions, coliseum challengers
and duel squads, 93% of its GE mobs notorious against a 12% baseline. The 1620
reading rests only on naming, since every member is one species. The 1890 one is
measured and gated.
...
from this dump's table - 24 actors under one display name, the shape of the primal
and single-creature races, sitting immediately past the table's last row - and
**1910** is five humanoids named at runtime - all five point at the client's
`kahen` variable-name sentinel, not at a missing translation. One block is not monsters at all: **1900 is 98 friendly NPCs that fight** -
...
content, which is what corroborates the imperial reading of 1800.
...
`sagittarius` in all three tiers** - the client has 7 of 7 in each, GE has 6 of 7 in each,
and the omission is the same rank every time across 12 client actors, which is a hole in
...
Two client facts came out of chasing one 1800 sub-run and they bound everything above.
**A display-name id is not the actor id with its 2 turned into a 3** - that parallel holds
...
The Garlean thread also turned up **a fourth GE evidence tier nothing here had mined**.
...
The quest infobox then turns each attribution into **the strongest level statement in the
set**. `Minimum Level` is filled on 263 of the 273 pages and behaves as a floor: the mob's
...
interpolating between other mobs. 50 rows carry a `quest_level_floor`, 28 of them `ge-gap`,
and six names no bracket could reach now have a level - including `nael van darnus` and
...
counts leave-one-out tests contaminated by the mob under test - the leak that first made
this look 84% accurate. Two other readings of the names were tested and rejected: place
prefixes like `Bloodshore` are real but cannot be told from ordinary adjectives by any
vocabulary in reach, and the actor-id sequence is **not** a level ladder (an id pair is
in level order 59% of the time).
...
- so those cells are blank by decision rather than omission. One scope caveat came out of
it and applies to the whole 468: the corpus holds no page at all for `nael van darnus`,
...
Reading the 17 names that own the highest id in their race then exposed a confound in
that gradient and produced the best hint in the file. **Ten of the 17 are in the `23`
allocation band**, which is by construction the numerically highest band a race can use,
so "late in the run" and "in the 23 band" overlap: measured within a single band the two
medians are 0.556 against 0.472, half the 0.178 seen across bands. And the 23 band turns
out to be **a level floor rather than a correlation** - every one of the 45 documented
...
rather than a named race. What the band selects for is now read too, and it is what the floor was
measuring: **the 23 band is instanced content.** 43 of the 47 documented band-23 mobs
...
general flanking hint. The roster is derived from the sightings each run rather than
written down, and `--verify` holds the band 90% inside its own top four zones.

Reading the other two bands then reframed all of that: **the band digit is a spawn
container**, and a mob actor id is `2 | band | race | variant`. 277 of the 280 documented
...
nothing else - which they do, on 7 references over 2 actors, too thin to carry the finding
but pointing the right way. `spawn_container` is published on both the crosscheck and the
gap table: **169 of the 270 gap names hold a band-22 actor**, which is the largest single
reason GE never wrote them up, and the 23-band level floor and dungeon shortlist are now
readable as properties of dungeon content rather than of a number.

Reading the three sighted band-22 mobs GE attached no leve to then produced a second
correlate and a new hint. `Frenzied Aurelia` is settled by GE's own `Notes` - it "Spawns
as part of the Limsa Lominsa Main Scenario Quest Shapeless Melody", an exact client quest
name - and the other two by **the level span, which owes nothing to the tag column**: a
band-22 mob spans 10 levels or more 0.480 of the time against 0.018 for a persistent one,
and 97 of those 120 wide spans are a multiple of 4, the shape of an evenly stepped
...
The 52 names that hint cannot reach then got the same treatment, and **the one-sided flank
clears the gate the level side failed**: over the 185 documented points that genuinely have
...
high, 14 below its low, 7 in a race where GE tagged no band-22 mob at all. Scoping that
measurement is the trap: run it over every point instead and it reads 0.358, which still
...
Chasing the last 7 - names in races GE tagged nothing in - then turned up a better source
than any interpolation here. **The client's own leve briefings name mobs**: `xtx_guildleve`
column 11 is the English leve name and column 15 the English objective, 610 rows carry both,
and 167 of those briefings name a client mob. `client_leve` on both mob tables attributes
...
the 14 that do not are read in the next paragraph.

For the 7 the answer is negative and now pinned as one: no briefing names any of them. Two
of them turn out to be attributed a tier up - `mob-quest-attribution.csv` puts `goblin
...
Reading the **29 documented mobs the client attributes and GE never tagged** then found a
defect in that column: most of them are species words, not individuals - `Balloon`,
...
That lane also produced the strongest check on the container reading, which until then
rested entirely on GE's tag column. **Folding the client's own attribution in as a second
...
Reading the 12 persistent-only mobs behind that rise then corrected the reading of it.
**All 12 are species words** - `Balloon`, `Basilisk`, `Cactuar`, `Sundrake` - reached because
...
and fails if the mixed merge adds no band-21 mob GE had missed, since a swap that changes
nothing tests nothing.

Reading all 15 mismatches in their own briefing sentences then found one real bug and six
kinds of prose. **`cassiopeia` was a place, not a mob** - all 13 of its attributions were
briefings naming `Cassiopeia Hollow` - so the client's own 907 place names now join the scan
as decoys that win positions without being attributed. That also caught `kobold`, attributed
...
and every one of those races does hold band-22 actors, `frightened cactuar` and `bloated
burble` among them, so they support the container reading rather than contradicting it. The
...
The same scan then ran over the **quest** side, which took two attempts. `xtx_quest` column 7
is the English description and is the wrong field - it attributes 12 species words whose single
...
The two sources then disagree about something that matters. On the half worth trusting - where
the client names an individual - **all 29 leve attributions hold a band-22 actor and only 38 of
55 quest attributions do**. 17 named individuals are quest targets with no leve deployment at
all (`Audhumbla`, `Cactuar Jack`, `Daddy Longlegs`, `Gluttonous Gertrude`). So **quests reuse
the persistent field population and leves spawn their own roster** - the question the container
sections left open. `--verify` gates the margin between the sources, since the reading is the
difference rather than either figure.

`dragon` closes the list. Its two briefings are `Heat of the Moment`, where an airship
"attacked by a pair of dragons" outruns them and the leve is recovering the cargo it
jettisoned, and `Nutritious Fishes`, which opens on "the effort spent slaying deadly dragons
and saving distressed damsels" as a figure of speech. Race 1068 is `Dragon` and holds four
...
with `--verify` pinning the count so a fifteenth would be one nobody has read.

That question also surfaced a corroboration nobody arranged. The triage causes in
`client-mobs-without-ge-page.csv` were assigned from the names before the band digit was read,
and **the two meaning "spawned by content" are entirely band 22** - all 52 `not-a-monster`
...
The three `race-name-only` names with no band-22 actor - `crab`, `dragon`, `zombie` - close
out. None is an unused shell: all three carry full `actorclass_graphic` rows indistinguishable
from their race-mates. **`dragon` belongs to a class of four**: 12 of the 95 named mob races
...
rosters. `crab` and `zombie` are ordinary mobs in well-populated races that GE never wrote up,
but both sit in an **actor-to-name transposition** - `crab` holds `beryl crab`'s expected name
...
name. The table carries no taxonomy column, and the two attempts at one both produced fake
buckets. **The only relation the client asserts is race, and every pair has it** - all 26 are
within one race and one band, now gated. Whether the two *names* look related is a question
about English, and a string test answers it for only 8 of 26; the other 18 are 1.0 renaming
inside a family (`dapper cadaver` against `zombie`, `bomb baron` against `grenade`) or a
notorious monster against its species. There is no "unrelated" class.

Because the client has no behavior data either, the same run resolves each mob's
...
are in the local corpus; all eight were harvested from the live wiki
and are pinned by revision id in `derived/mob-behavior-codes.csv`. `S` is sight and
...
actor-id prefix turns out to be a real division: **prefix `16` is the merchant space**,
...
`xtx_displayName` turned out to be a **five-language** table with a singular and a plural
per language - 1 Japanese, 2/3 English, 7/9 German, 14/15 French, 20 Chinese - and reading
only English is what made 21 GE NPCs look absent. **1.0's locales did not name Grand Company
...
The 61 that do not join split three ways, and the first is corroboration rather than a
gap: **34 `mob-range-actor`** whose actor sits in a monster range - 14 rental chocobos and
**the 12 hamlet militia the 1900-block reading leans on**, now attested from GE's NPC
...
(146), not a grouping label - and records a research-time cross-reference to
...
- Two client-side interpretations remain unresolved and require a decomp of
  ...
  client dump, and `weapon.141`'s runtime use (a displayed bow-plus-arrow total,
  or a no-ammunition fallback) is not decidable from the sheets. The pcap corpus
