...
- `sources/lodestone-manual/objects/images/<slug>/*` - 330 page screenshots and UI diagrams, referenced
  inline from each `.md`.
- Master archive: the full `Old Lodestone.rar` (all 208 saved pages, incl. the
  ~170 news / lore / patch-note / event pages **not** transcribed here) is
  cold-stored at `archives/lodestone-manual/Old Lodestone.rar`
  (gitignored); sha256 and restore details are in `manifest.yaml` sources.
