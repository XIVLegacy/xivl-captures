...
Hosted CI detects the missing corpus and runs the gate in public-shape mode.
...
is not a durable evidence record. The
[evidence and claims guide](docs/ai_agents/evidence-and-claims.md) is the
...
from a public checkout when the gate ran in public-shape mode.
...
CI must be green before merge. Complete the pull request template with the
stable evidence ids, changed artifacts, and verification result a reviewer
needs. Commit subjects are one line and 50 characters or fewer, with no body,
trailers, or attribution lines.
...
Choose the issue form that matches the report and include stable ids,
