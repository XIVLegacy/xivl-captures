...
VERIFIED_PRODUCTS = tuple(
    ...
    + [
        ...
        "studies/lobby-handshake-triage/derived/lobby-acknowledgement-structure.json",
    ]
)
UNIT_TEST_MODULES = (
    ...
    "tools.tests.test_extract_lobby_acknowledgement_schema",
)
...
LOBBY_ACKNOWLEDGEMENT = TOOLS / "extractors" / "extract_lobby_acknowledgement_schema.py"
...
def do_public_check() -> int:
    ...
    ok = run([LOBBY_ACKNOWLEDGEMENT, "--check", "--public-shape"],
             "public lobby acknowledgement fixture")
    results.append(("sanitized lobby acknowledgement structure", "validate", ok,
...
def do_check() -> int:
    ...
    ok = run([LOBBY_ACKNOWLEDGEMENT, "--check"],
             "extract_lobby_acknowledgement_schema.py --check")
    results.append(("sanitized lobby acknowledgement structure", "regen", ok,
                    "" if ok else "regenerated bytes differ, see output above"))
...
def do_write() -> int:
    ...
    ok = run([LOBBY_ACKNOWLEDGEMENT], "extract_lobby_acknowledgement_schema.py")
    results.append(("sanitized lobby acknowledgement structure", "write", ok,
                    "" if ok else "generation failed, see output above"))
