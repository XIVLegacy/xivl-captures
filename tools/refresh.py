...
PCAP_CSV_PRODUCTS = ["spawn_observations.csv"]
...
UNIT_TEST_MODULES = (
    ...
    "tools.tests.test_extract_spawn_observations",
    ...
    "tools.tests.test_extract_monster_actor_class_paths",
...
MONSTER_ACTOR_CLASS_PATHS = (
    TOOLS / "extractors" / "extract_monster_actor_class_paths.py"
)
...
def do_check() -> int:
    ...
    ok = run(
        [MONSTER_ACTOR_CLASS_PATHS, "--check"],
        "extract_monster_actor_class_paths.py --check",
    )
    results.append(
        (
            "monster actor-class path join",
            "regen",
            ok,
            "" if ok else "regenerated bytes differ, see output above",
        )
...
def do_write() -> int:
    ...
    ok = run(
        [MONSTER_ACTOR_CLASS_PATHS],
        "extract_monster_actor_class_paths.py",
    )
    results.append(
        (
            "monster actor-class path join",
