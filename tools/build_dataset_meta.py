...
def build_all_meta() -> dict[str, dict]:
    ...
    for json_path in sorted(DATA_DIR.glob("*.json")):
        ...
        if name in PROMOTED_DATASETS:
            ...
            out[name] = build_generated_meta(
                name, pipelines_by_output[name], corpus_hash
            )
        else:
            print(
...
def do_check() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        for name, meta in all_meta.items():
            ...
            if tmp_path.read_bytes() != committed.read_bytes():
                failures.append(
                    f"derived/{name}.meta.yaml is stale (regenerated bytes differ)"
                )
    ...
    for sidecar in sorted(DATA_DIR.glob("*.meta.yaml")):
        ...
        if name not in known:
            failures.append(
                f"derived/{sidecar.name} has no matching derived/{name}.json"
            )
