#!/usr/bin/env python3
"""Generate or verify every canonical pcap-derived product in one process."""
...
def _run_group(group: str, stage: Path) -> int:
    ...
    commands = {
        ...
        "request_response_pairs": (
            extract_request_response_pairs.main,
            ["--out", out(group)],
        ),
        "gam_keys": (extract_gam_keys.main, ["--out", out(group)]),
        "spawn_observations": (
            extract_spawn_observations.main,
            ["--out", out(group), "--csv-out", csv_out(group)],
        ),
        "gam_hash_names": (
...
def _public_check(selected: set[str], output_dir: Path) -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if "spawn_observations" in selected:
            errors.extend(
                extract_spawn_observations.validate_csv(
                    output_dir / "spawn_observations.json",
                    output_dir / "spawn_observations.csv",
                )
            )
        ...
        for group in ("gam_hash_names", "payload_layouts"):
            ...
            if group == "gam_hash_names":
                status = _invoke_main(
                    ...
                    [
                        "--in",
                        str(output_dir / "gam_keys.json"),
                        "--out",
                        str(stage / "gam_hash_names.json"),
                    ],
                )
            else:
                status = _invoke_main(
                    ...
                    [
                        "--in",
                        str(output_dir / "payload_samples.json"),
                        "--out",
                        str(stage / "payload_layouts.json"),
                    ],
                )
            if status:
                errors.append(
                    f"{group} public-shape reducer failed with status {status}"
                )
                continue
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for group, outputs in GROUPS:
                ...
                try:
                    ...
                    Exception
                ) as exc:  # Atomic publication requires reducer isolation.
            ...
            for product in selected:
                ...
                if product == "spawn_observations":
                    staged_csv = stage / f"{product}.csv"
                    if not staged_csv.is_file():
                        errors.append(
                            f"{product} reducer produced no staged CSV output"
                        )
            ...
            if not errors and check:
                for product in selected:
                    ...
                    if not committed.is_file():
                        ...
                        errors.append(
                            f"derived/{product}.json regenerated bytes differ"
                        )
                    if product == "spawn_observations":
                        staged_csv = stage / f"{product}.csv"
                        committed_csv = output_dir / f"{product}.csv"
                        if not committed_csv.is_file():
                            errors.append(f"committed product missing: {committed_csv}")
                        elif staged_csv.read_bytes() != committed_csv.read_bytes():
                            errors.append(
                                f"derived/{product}.csv regenerated bytes differ"
                            )
            ...
            if not errors and not check:
                ...
                for product in selected:
                    ...
                    if product == "spawn_observations":
                        staged_csv = stage / f"{product}.csv"
                        target_csv = output_dir / f"{product}.csv"
                        pending_csv = output_dir / f".{product}.csv.pending"
                        shutil.copyfile(staged_csv, pending_csv)
                        publish.append((pending_csv, target_csv))
