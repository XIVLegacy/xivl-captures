...
def _validate_reads() -> list[str]:
    ...
    if any(path.name == "login.pcapng" for path in counts):
        errors.append("login.pcapng was read")
    if len(expected) != 53:
        errors.append(f"expected corpus selection has {len(expected)} pcaps, not 53")
    return errors
...
def _public_check(selected: set[str], output_dir: Path) -> int:
    ...
    print("pcap reads: 0 unique, 0 total; login.pcapng=0 (public shape)")
    for error in errors:
...
def execute(
    ...
    *,
    ...
    login_reads = sum(
        count for path, count in LAST_READ_COUNTS.items() if path.name == "login.pcapng"
    )
    print(
        f"pcap reads: {len(LAST_READ_COUNTS)} unique, {total_reads} total; "
        f"login.pcapng={login_reads}"
    )
    for error in errors:
        print(f"ERROR: {error}", file=sys.stderr)
    return 1 if errors else 0
