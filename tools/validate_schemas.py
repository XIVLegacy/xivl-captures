...
def load_evidence_classes() -> set[str]:
    doc = yaml.safe_load(
        (SCHEMAS_DIR / "evidence-classes.yaml").read_text(encoding="utf-8")
    )
...
def check_lobby_record_census(results: list) -> None:
    path = (
        STUDIES_DIR / "lobby-handshake-triage" / "derived" / "lobby-record-census.json"
    )
    ...
    try:
        ...
        results.append(
            ("schema: sanitized decrypted lobby record census", False, str(exc))
        )
        return
    validate_doc(
        document, schema, "schema: sanitized decrypted lobby record census", results
    )
...
def check_id_ceiling(value: str, context: str, results: list) -> None:
    ...
    results.append(
        (
            context,
            ok,
            "" if ok else f"id {value!r} exceeds the {ID_CEILING}-char ceiling",
        )
    )
...
def check_sources(results: list, evidence_classes: set[str]) -> dict[str, dict]:
        ...
        if "evidence_class" in doc:
            ...
            results.append(
                (
                    ...
                    ""
                    if ok
                    else f"evidence_class {doc['evidence_class']!r} not in schemas/evidence-classes.yaml",
                )
            )

        if "id" in doc:
            check_id_ceiling(
                doc["id"], f"id ceiling: sources/{source_dir.name} id", results
            )
    ...
    print(
        f"objects hash-verify timing: {total_files} files across {len(timing)} "
        f"sources in {total_seconds:.2f}s",
        file=sys.stderr,
    )
    return manifests
...
def check_retail_contracts(results: list) -> None:
    ...
    contracts = (
        ...
        (
            CONFIG_DIR / "retail_pcap_check.json",
            SCHEMAS_DIR / "retail_pcap_check.schema.json",
        ),
    )
    for document_path, schema_path in contracts:
        ...
        try:
            ...
            results.append(
                (label, False, f"retail contract unreadable ({exc.__class__.__name__})")
            )
            continue
    ...
    if attestation_path.exists():
        ...
        try:
            ...
            results.append(
                (label, False, f"attestation unreadable ({exc.__class__.__name__})")
            )
        else:
            validate_doc(document, schema, label, results)
...
def check_source_boundary(source_dir: Path, doc: dict, results: list) -> None:
    ...
    label_stray = (
        f"boundary: sources/{source_dir.name} no stray files beside manifest.yaml"
    )
    strays = sorted(
    ...
    if strays:
        results.append(
            (label_stray, False, f"stray path(s) outside objects/: {', '.join(strays)}")
        )
    else:
        results.append((label_stray, True, ""))

    objects_dir = (
        pcap_objects_dir(source_dir / "objects")
        if source_dir.name == "pcap-1.23b"
    ...
    if original_state == "cold-stored":
        ...
        results.append(
            (label, ok, "" if ok else "objects/ exists alongside cold-stored storage")
        )
    ...
    results.append(
        (
            ...
            ""
            if ok
            else f"distribution={distribution!r}, storage.original_state={original_state!r}",
        )
    )


def check_source_objects(
    source_dir: Path, doc: dict, results: list, timing: list
) -> None:
    ...
    if not objects_dir.is_dir():
        ...
        if original_state == "local-only":
            ok = not members
            results.append(
                (label, ok, "" if ok else "local-only source must not carry members")
            )
            return
...
def check_dataset_meta(results: list, evidence_classes: set[str]) -> set[str]:
    ...
    for sidecar in sorted(DATA_DIR.glob("*.meta.yaml")):
        ...
        if "dataset" in doc:
            check_id_ceiling(
                doc["dataset"],
                f"id ceiling: derived/{sidecar.name} dataset id",
                results,
            )
            ok = doc["dataset"] == name
            results.append(
                (
                    ...
                    ""
                    if ok
                    else f"dataset field {doc['dataset']!r} != filename stem {name!r}",
                )
            )
...
def check_data_sidecar_pairing(results: list, sidecar_names: set[str]) -> None:
    ...
    if missing_sidecars:
        note_parts.append(
            f"derived/*.json missing a sidecar: {', '.join(missing_sidecars)}"
        )
    if orphan_sidecars:
        note_parts.append(
            f"sidecar with no derived/*.json: {', '.join(orphan_sidecars)}"
        )
    results.append(
        ("derived/*.json <-> derived/*.meta.yaml pairing", ok, "; ".join(note_parts))
    )
...
def check_pipelines(results: list) -> None:
    ...
    for path in sorted(PIPELINES_DIR.glob("*.yaml")):
        ...
        if "id" in doc:
            check_id_ceiling(
                doc["id"], f"id ceiling: pipelines/{path.name} id", results
            )
        ...
        if runner_tool:
            ...
            if runner_ok:
                ...
                version_ok = (
                    code_version is not None and code_version == declared_version
                )
                results.append(
                    (
        ...
        for output in doc.get("outputs", []):
            ...
            check_id_ceiling(
                out_id, f"id ceiling: pipelines/{path.name} output {out_id!r}", results
            )
                ...
                if "source" in inp:
                    ...
                    results.append(
                        (
                            ...
                            ""
                            if input_ok
                            else f"derived/{input_id}.json does not exist",
                        )
                    )
...
def check_studies(results: list, evidence_classes: set[str]) -> None:
    ...
    for study_dir in sorted(p for p in STUDIES_DIR.iterdir() if p.is_dir()):
        ...
        if "id" in doc:
            check_id_ceiling(
                doc["id"], f"id ceiling: studies/{study_dir.name} id", results
            )
...
def check_party_marker_field_census(results: list) -> None:
    ...
    path = (
        STUDIES_DIR / "party-marker-018d-chronology" / "derived" / "field-census.json"
    )
...
def check_world_party_chat_00c9_contract(results: list) -> None:
    ...
    path = (
        STUDIES_DIR / "world-party-chat-00c9-contract" / "derived" / "accounting.json"
    )
...
def check_path_lengths(results: list) -> None:
    ...
    try:
        ...
        results.append(
            (
                "path ceiling: git ls-files",
                False,
                f"could not enumerate tracked files: {exc}",
            )
        )
        return
    too_long = [
        line for line in proc.stdout.splitlines() if line and len(line) > PATH_CEILING
    ]
    ok = not too_long
    note = (
        ""
        if ok
        else f"{len(too_long)} path(s) exceed {PATH_CEILING} chars: "
        + ", ".join(too_long[:5])
    )
    results.append(
        (f"path ceiling: every tracked file <= {PATH_CEILING} chars", ok, note)
    )
