...
class PropertyStreamCatalogTests(unittest.TestCase):
    def test_property_and_target_marker(self):
        ...
        rows, declared, terminated = MODULE.parse_records(block)
    ...
    def test_zero_terminator(self):
        rows, declared, terminated = MODULE.parse_records(bytes([1, 0]))
        self.assertEqual((rows, declared, terminated), ([], 1, True))

    def test_incomplete_tail_is_not_promoted(self):
        rows, declared, terminated = MODULE.parse_records(bytes([3, 4, 1, 2]))
