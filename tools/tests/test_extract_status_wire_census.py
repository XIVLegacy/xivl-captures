...
class StatusWireCensusTests(unittest.TestCase):
    ...
    def test_projections_overlap_without_labels(self):
        self.assertEqual(
            census.unpack_status_word(223263),
            {
                "chant_kind_1": 6,
                "chant_kind_2": 8,
                "object_bits_8_11": 8,
    ...
    def test_retransmission_overlap_is_counted(self):
        ...
        try:
            ...
            self.assertEqual(
                census._raw_stream_accounting(Path("x"), connection, "s2c"), (4, 4)
            )
        finally:
    ...
    def test_malformed_status_shapes_are_excluded(self):
        valid = bytes(
            census.INNER_HEADER_LEN
            + census.GAME_PREAMBLE_SIZE
            + census.APPLICATION_SIZE
        )
        self.assertEqual(
            census.decode_status_ids(71, valid), (None, "unexpected_subevent_size")
        )
        self.assertEqual(
            census.decode_status_ids(72, valid[:-1]),
            (None, "unexpected_application_shape"),
        )
    ...
    def test_accounting_matches_public_schema(self):
        ...
        accounting = json.loads(
            (census.OUT / "accounting.json").read_text(encoding="ascii")
        )
        schema = json.loads(
            (ROOT / "schemas" / "status-wire-census.schema.json").read_text(
                encoding="ascii"
            )
        )
    ...
    def test_schema_rejects_sensitive_nested_field(self):
        ...
        capture = next(
            value
            for value in mutated["per_capture_chronology"].values()
            if value["events"]
        )
