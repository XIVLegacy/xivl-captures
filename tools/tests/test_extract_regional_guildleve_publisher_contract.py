...
class RegionalGuildlevePublisherContractTest(unittest.TestCase):
    def test_timeline_preserves_frame_order_within_capture_packet(self):
        rows = [
            {
                "stage": "event-retirement",
                "capture_packet_index": 5444,
                "frame_stream_offset": 426300,
                "subevent_offset": 0,
                "direction": "s2c",
            },
            {
                "stage": "content-group-retirement-4",
                "capture_packet_index": 5444,
                "frame_stream_offset": 426046,
                "subevent_offset": 144,
                "direction": "s2c",
            },
            {
                "stage": "content-group-retirement-7",
                "capture_packet_index": 5444,
                "frame_stream_offset": 426046,
                "subevent_offset": 80,
                "direction": "s2c",
    ...
    def test_lua_values_decode_known_scalar_tags(self):
        values, unsupported = decode_lua_values(
            bytes.fromhex("00 000030c3 03 04 05 0f")
        )
        self.assertEqual(
            values,
            [
