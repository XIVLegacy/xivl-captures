...
class PartyMarkerChronologyTests(unittest.TestCase):
    def test_decodes_client_reads_and_labeled_hypothesis(self):
        decoded, reason = marker.decode_application(
            marker.EXPECTED_SUBEVENT_SIZE, sub_body(application())
        )
    ...
    def test_rejects_count_beyond_reserved_rows(self):
        decoded, reason = marker.decode_application(
            marker.EXPECTED_SUBEVENT_SIZE,
            sub_body(application(marker.RECORD_CAPACITY + 1)),
        )
        self.assertIsNone(decoded)
    ...
    def test_rejects_nonzero_reserved_tail(self):
        ...
        data[-1] = 1
        decoded, reason = marker.decode_application(
            marker.EXPECTED_SUBEVENT_SIZE, sub_body(data)
        )
        self.assertIsNone(decoded)
    ...
    def test_snapshot_shapes_are_chronology_only(self):
        first = {
            ...
            "lane": "main",
            "header": (1, 2, 3),
            "records": ((4, 5, 6, 1.0, 2.0, 3.0, 4.0),),
            "count": 1,
            "snapshot_key": b"first",
        }
        repeated = dict(first)
        changed = {
            **first,
            "records": ((4, 5, 6, 2.0, 2.0, 3.0, 4.0),),
            "snapshot_key": b"changed",
        }
    ...
    def test_retransmission_overlap_is_counted_once(self):
        packets = [
            ...
            / Raw(b"abcd"),
            IP(src="203.0.113.10", dst="203.0.113.20")
            / TCP(sport=54992, dport=50000, seq=104)
            / Raw(b"efgh"),
        ]
        connection = {
        ...
        with patch.object(marker, "read_packets", return_value=packets):
            self.assertEqual(
                marker._segment_accounting(Path("unused"), connection, "s2c"), (4, 0)
            )

    def test_accounting_schema_rejects_unknown_fields(self):
        schema = json.loads(
            (ROOT / "schemas" / "party-marker-chronology.schema.json").read_text(
                encoding="utf-8"
            )
        )
        accounting = json.loads(
            (
                ROOT / "studies" / marker.STUDY_ID / "derived" / "accounting.json"
    ...
    def test_public_columns_exclude_sensitive_raw_surfaces(self):
        fields = (
            marker.OCCURRENCE_FIELDS + marker.RECORD_FIELDS + marker.NEIGHBOR_FIELDS
        )
