...
def sub_body(header_clock=100, reserved=0, subopcode=0x12, value=30):
    data = bytearray(
        clock.INNER_HEADER_LEN + clock.PACKET_HEADER_TAIL_SIZE + clock.APPLICATION_SIZE
    )
    struct.pack_into("<II", data, clock.INNER_HEADER_LEN, header_clock, reserved)
    struct.pack_into(
        "<II",
        data,
        clock.INNER_HEADER_LEN + clock.PACKET_HEADER_TAIL_SIZE,
        subopcode,
        value,
    )
    return bytes(data)
...
class Map0193ClockContractTests(unittest.TestCase):
    ...
    def test_retransmission_overlap_is_counted_once(self):
        ...
        with patch.object(clock, "read_packets", return_value=packets):
            self.assertEqual(
                clock._segment_accounting(Path("unused"), connection, "s2c"), (4, 0)
            )
    ...
    def test_accounting_schema_rejects_mutation(self):
        schema = json.loads(
            (ROOT / "schemas" / "map-0193-clock-contract.schema.json").read_text(
                encoding="utf-8"
            )
        )
        accounting = json.loads(
            (
                ROOT / "studies" / clock.STUDY_ID / "derived" / "accounting.json"
