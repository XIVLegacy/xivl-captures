...
ACCOUNTING_FIELDS = (
    ...
    "link_record_count",
    "framed_event_count",
    "exact_item_link_count",
    "bounded_candidate_count",
    "missing_carrier_count",
    "repeated_aggregate_event_count",
    "retransmitted_segment_count",
...
MATRIX_FIELDS = (
    ...
    "linked_item_slot",
    "item_package_code",
    "item_to_link_distance_frames",
    "item_to_link_distance_subevents",
    "item_to_link_distance_us",
    "before_property_frame",
    "before_distance_frames",
    "before_distance_us",
    "after_property_frame",
    "after_distance_frames",
    "after_distance_us",
    "changed_property_count",
    "repeated_aggregate_of",
    "boundary_note",
)
PROPERTY_FIELDS = (
    "event_id",
...
def _actor_labels(
    events: list[dict], properties: list[dict[str, str]]
) -> dict[int, str]:
    labels: dict[int, str] = {}
    actor_ids = []
    for row in events:
        actor_ids.extend(
            (int(row["source_actor_id"]), int(row["destination_actor_id"]))
        )
...
def _parse_link_rows(sub: bytes, opcode: int) -> list[dict[str, int]]:
    ...
    for index in range(count):
        equipment_slot, item_slot, package_code = struct.unpack_from(
            "<HHH", payload, index * 6
        )
        rows.append(
            {
                ...
                "item_slot": item_slot,
                "item_package_code": package_code,
            }
        )
    return rows
...
def _scan_capture(
    path: Path,
) -> tuple[list[dict], int, Counter[int], dict[tuple[int, int], int]]:
    events: list[dict] = []
    ...
    observed = (
        {SET_BEGIN, SET_END, CHANGE_BEGIN, CHANGE_END}
        | set(ITEM_COUNTS)
        | set(LINK_COUNTS)
    )
    for lane_index, lane in enumerate(lanes):
        for frame_index, frame in enumerate(
            parse_outer_frames(lane["streams"].get("s2c", b""))
        ):
            frame_times[(lane_index, frame_index)] = struct.unpack(
                "<Q", frame["timestamp"]
            )[0]
                    ...
                    if len(sub) >= INNER_HEADER_LEN:
                        ...
                        if opcode in observed:
                            base = {
                                ...
                                "ordinal": ordinal,
                                "opcode": opcode,
                                "source_actor_id": struct.unpack_from(
                                    "<I", body, offset + 4
                                )[0],
                                "destination_actor_id": struct.unpack_from(
                                    "<I", body, offset + 8
                                )[0],
                            }
                            if opcode in ITEM_COUNTS:
                                ...
                                if len(payload) != 8:
                                    raise ValueError(
                                        f"{path.name}: unexpected 0x0146 payload size"
                                    )
                                actor_id, set_size, set_code = struct.unpack(
                                    "<IHH", payload
                                )
                                base.update(
                                    {
...
def _property_projection(
    ...
    *,
    ...
    if not candidates:
        return []
    nearest_frame = (max if before else min)(
        int(row["frame_index"]) for row in candidates
    )
    ...
    for lane_index, lane_events in sorted(by_lane.items()):
        ...
        for index, event in enumerate(lane_events):
            ...
            previous_change = next(
                (
                    lane_events[index]
                    for index in range(begin_index - 1, -1, -1)
                    if lane_events[index]["opcode"] in {CHANGE_BEGIN, CHANGE_END}
                ),
                None,
            )
            next_change = next(
                (
                    lane_events[index]
                    for index in range(end_index + 1, len(lane_events))
                    if lane_events[index]["opcode"] in {CHANGE_BEGIN, CHANGE_END}
                ),
                None,
            )
            transactions.append(
                {
                    ...
                    "change_begin": previous_change
                    if previous_change and previous_change["opcode"] == CHANGE_BEGIN
                    else None,
                    "change_end": next_change
                    if next_change and next_change["opcode"] == CHANGE_END
                    else None,
                }
            )
            begin_index = None
...
def extract() -> tuple[list[dict], list[dict], list[dict], dict[str, int]]:
    ...
    if len(corpus_paths) != 54:
        raise ValueError(
            f"expected canonical 54-capture corpus, found {len(corpus_paths)}"
        )
    ...
    for path in corpus_paths:
        ...
        inventory_state: dict[int, dict[tuple[int, int], list[tuple[dict, dict]]]] = (
            defaultdict(dict)
        )
        ...
        for transaction in transactions:
            ...
            link_events = [
                row for row in transaction["members"] if row["opcode"] in LINK_COUNTS
            ]
            item_candidates: dict[tuple[int, int], list[tuple[dict, dict]]] = (
                defaultdict(list)
            )
            item_rows = _item_rows_with_package(transaction)
            for item_event, item, package_code in item_rows:
                item_candidates[(package_code, int(item["item_slot"]))].append(
                    (item_event, item)
                )
            ...
            before = _property_projection(
                **projection_args, boundary=_event_order(begin), before=True
            )
            after = _property_projection(
                **projection_args, boundary=_event_order(end), before=False
            )
            ...
            changed_hashes = sorted(
                prop_hash
                for prop_hash in before_by_hash.keys() & after_by_hash.keys()
                if before_by_hash[prop_hash]["value_hex"]
                != after_by_hash[prop_hash]["value_hex"]
            )
            before_frame = before[0]["frame_index"] if before else ""
            after_frame = after[0]["frame_index"] if after else ""
            before_distance = (
                int(begin["frame_index"]) - int(before_frame) if before else ""
            )
            after_distance = int(after_frame) - int(end["frame_index"]) if after else ""
            lane_index = int(transaction["lane_index"])
            before_distance_us = (
                (
                    frame_times[(lane_index, int(begin["frame_index"]))]
                    - frame_times[(lane_index, int(before_frame))]
                )
                if before
                else ""
            )
            after_distance_us = (
                (
                    frame_times[(lane_index, int(after_frame))]
                    - frame_times[(lane_index, int(end["frame_index"]))]
                )
                if after
                else ""
            )
            ...
            if any(event["opcode"] != 0x014D for event in link_events):
                fingerprint = tuple(
                    (
                        ...
                        link["equipment_slot"],
                        link["item_slot"],
                        link["item_package_code"],
                    )
                    for event, link in link_rows
                )
            ...
            common = {
                ...
                "set_begin_frame": begin["frame_index"],
                "set_begin_subevent": begin["subevent_index"],
                "set_end_frame": end["frame_index"],
                "set_end_subevent": end["subevent_index"],
                "change_begin_frame": transaction["change_begin"]["frame_index"]
                if transaction["change_begin"]
                else "",
                "change_begin_subevent": transaction["change_begin"]["subevent_index"]
                if transaction["change_begin"]
                else "",
                "change_end_frame": transaction["change_end"]["frame_index"]
                if transaction["change_end"]
                else "",
                "change_end_subevent": transaction["change_end"]["subevent_index"]
                if transaction["change_end"]
                else "",
                "before_property_frame": before_frame,
                "before_distance_frames": before_distance,
                "before_distance_us": before_distance_us,
                "after_property_frame": after_frame,
                "after_distance_frames": after_distance,
                "after_distance_us": after_distance_us,
                "changed_property_count": len(changed_hashes),
            }
            if not link_rows:
                for item_event, item, package_code in item_rows:
                    lane_state[(package_code, int(item["item_slot"]))] = [
                        (item_event, item)
                    ]
                capture_matrix.append(
                    {
                        **_blank(MATRIX_FIELDS),
                        **common,
                        "classification": "MISSING-CARRIER",
                        "join_status": "no-equipment-link",
            ...
            for link_event, link in link_rows:
                ...
                row = {
                    ...
                    **common,
                    ...
                    "join_status": f"exact-{match_origin}-item-link"
                    if match
                    else ("ambiguous-item-link" if matches else "missing-item"),
                    "link_opcode": f"0x{link_event['opcode']:04X}",
                    "link_frame": link_event["frame_index"],
                    "link_subevent": link_event["subevent_index"],
                    "link_record": link["link_record"],
                    "equipment_slot": link["equipment_slot"],
                    "linked_item_slot": link["item_slot"],
                    "item_package_code": link["item_package_code"],
                    "repeated_aggregate_of": repeated_of,
                    "boundary_note": "same-lane s2c order is correlation evidence, not causality",
                }
                if match:
                    item_event, item = match
                    item_frame_distance = int(link_event["frame_index"]) - int(
                        item_event["frame_index"]
                    )
                    item_subevent_distance = (
                        int(link_event["subevent_index"])
                        - int(item_event["subevent_index"])
                        if not item_frame_distance
                        else ""
                    )
                    item_distance_us = (
                        frame_times[(lane_index, int(link_event["frame_index"]))]
                        - frame_times[(lane_index, int(item_event["frame_index"]))]
                    )
                    row.update(
                        {
                            "item_opcode": f"0x{item_event['opcode']:04X}",
                            "item_frame": item_event["frame_index"],
                            "item_subevent": item_event["subevent_index"],
                            "item_record": item["item_record"],
                            "catalog_item_id": f"0x{int(item['item_id']):08X}",
                ...
                if link_event["opcode"] == 0x014D and match:
                    for prop_hash in sorted(
                        ...
                        if old and new:
                            comparison_status = (
                                "changed"
                                if old["value_hex"] != new["value_hex"]
                                else "unchanged"
                            )
                        else:
                        ...
                        property_rows.append(
                            {
                                ...
                                "before_record_index": old["record_index"]
                                if old
                                else "",
                                ...
                                "after_record_index": new["record_index"]
                                if new
                                else "",
        ...
        accounting_rows.append(
            {
                ...
                "set_begin_count": opcode_counts[SET_BEGIN],
                "set_end_count": opcode_counts[SET_END],
                "change_begin_count": opcode_counts[CHANGE_BEGIN],
                "change_end_count": opcode_counts[CHANGE_END],
                **{
                    f"opcode_0x{opcode:04x}_count": opcode_counts[opcode]
                    for opcode in range(0x0148, 0x0152)
                },
                "item_packet_count": sum(
                    opcode_counts[opcode] for opcode in ITEM_COUNTS
                ),
                "item_record_count": sum(
                    len(event.get("items", []))
                    for event in events
                    if event["opcode"] in ITEM_COUNTS
                ),
                "link_packet_count": sum(
                    opcode_counts[opcode] for opcode in LINK_COUNTS
                ),
                "link_record_count": sum(
                    len(event.get("links", []))
                    for event in events
                    if event["opcode"] in LINK_COUNTS
                ),
                "framed_event_count": len(transactions),
                "exact_item_link_count": sum(
                    str(row["join_status"]).startswith("exact-")
                    for row in capture_matrix
                ),
                "bounded_candidate_count": sum(
                    row["classification"] == "BOUNDED-CANDIDATE"
                    for row in capture_matrix
                ),
                "missing_carrier_count": sum(
                    row["classification"] == "MISSING-CARRIER" for row in capture_matrix
                ),
                "repeated_aggregate_event_count": repeated_aggregate_count,
                "retransmitted_segment_count": _retransmitted_segments(path),
                "excluded_0x018f_0x0191_count": sum(
                    opcode_counts[opcode] for opcode in EXCLUDED_NEARBY
                ),
            }
        )
    ...
    summary = {
        ...
        "exact_transitions": sum(
            row["classification"] == "EXACT-TRANSITION" for row in matrix_rows
        ),
        "bounded_candidates": sum(
            row["classification"] == "BOUNDED-CANDIDATE" for row in matrix_rows
        ),
        "aggregate_snapshots": sum(
            row["classification"] == "AGGREGATE-SNAPSHOT" for row in matrix_rows
        ),
        "missing_carriers": sum(
            row["classification"] == "MISSING-CARRIER" for row in matrix_rows
        ),
        "property_joins": len(property_rows),
        "retransmitted_segments": sum(
            int(row["retransmitted_segment_count"]) for row in accounting_rows
        ),
        "repeated_aggregate_events": sum(
            int(row["repeated_aggregate_event_count"]) for row in accounting_rows
        ),
        "excluded_nearby_packets": sum(
...
def _evidence_map_bytes(matrix: list[dict], summary: dict[str, int]) -> bytes:
    ...
    for row in exact:
        lines.append(
            f"| {row['event_id']} | `{row['capture']}` | {row['equipment_slot']} | `{row['catalog_item_id']}` | {row['changed_property_count']} | {row['before_property_frame']} -> {row['after_property_frame']} |"
        )
    lines.extend(
        [
            "",
    ...
    for row in candidates:
        lines.append(
            f"| {row['event_id']} | `{row['capture']}` | {row['equipment_slot']} | `{row['catalog_item_id'] or 'missing'}` | {row['join_status']} | {row['before_property_frame'] or 'missing'} -> {row['after_property_frame'] or 'missing'} |"
        )
    lines.extend(
        [
            "",
...
def main() -> int:
    ...
    print(
        ...
        + f" {len(outputs)} equipment-transition artifacts"
    )
