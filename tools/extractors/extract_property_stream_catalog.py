...
def parse_records(block: bytes) -> tuple[list[dict], int, bool, int]:
    ...
    while pos < end:
        ...
        row = {
            "stream_offset": pos,
            "target_marker": target or "",
            "property_hash": f"0x{prop_hash:08x}",
            "value_width": width,
            "value_hex": value.hex(),
            "value_u_le": int.from_bytes(value, "little"),
            "value_f32": "",
        }
        if width == 4:
...
def scan() -> tuple[list[dict], dict]:
            ...
            for frame_index, frame in enumerate(parse_outer_frames(blob)):
                ...
                while off + SUB_EVENT_HEADER_LEN <= len(body):
                    ...
                    if event_type == SUB_EVENT_CLASS_ACTOR_WRAPPED:
                        ...
                        if (
                            len(sub) >= APP_OFFSET
                            and struct.unpack_from("<H", sub, 2)[0] == OPCODE
                        ):
                            if size != 168 or len(sub[APP_OFFSET:]) != 136:
                                raise ValueError(
                                    f"{capture.name}: unexpected 0x0137 shape {size}/{len(sub[APP_OFFSET:])}"
                                )
                            block = sub[APP_OFFSET:]
                            parsed, declared, terminated, consumed = parse_records(
                                block
                            )
                            # Reconcile with the retained canonical parser before promoting detailed rows.
                            canonical, _, canonical_declared = parse_property_block(
                                sub[APP_OFFSET:]
                            )
                            if canonical_declared != declared or len(canonical) != len(
                                parsed
                            ):
                                raise ValueError(
                                    f"{capture.name}: parser reconciliation failed"
                                )
                            for expected, actual in zip(canonical, parsed):
                                if (
                                    expected["idHex"] != actual["property_hash"]
                                    or expected["size"] != actual["value_width"]
                                    or expected["valueHex"] != actual["value_hex"]
                                ):
                                    raise ValueError(
                                        f"{capture.name}: property row reconciliation failed"
                                    )
                            packet_count += 1
                            ...
                            destination_actor = struct.unpack_from("<I", body, off + 8)[
                                0
                            ]
                                ...
                                rows.append(
                                    {
                                        ...
                                        "scenario_id": scenarios.get(
                                            capture.name, "unassigned"
                                        ),
                                        "lane_index": lane_index,
                                        "frame_index": frame_index,
                                        "subevent_index": sub_index,
                                        "packet_index": packet_index,
                                        "record_in_packet": record_in_packet,
                                        "source_actor_id": source_actor,
                                        "destination_actor_id": destination_actor,
                                        **row,
                                    }
                                )
                            packet_index += 1
    ...
    accounting = {
        "schema_version": 1,
        "carrier_opcodes": ["0x0137"],
        "packet_count": packet_count,
        "record_count": len(rows),
        "distinct_hashes": len(by_hash),
        "captures": len(packet_by_capture),
        "scenarios": len({r["scenario_id"] for r in rows}),
        "width_distribution": {
            str(k): v
            for k, v in sorted(Counter(r["value_width"] for r in rows).items())
        },
        "declared_total_distribution": {
            str(k): v for k, v in sorted(declared_totals.items())
        },
        ...
        "boundaries": [
            "value_u_le and value_f32 are parallel packet interpretations, not promoted property semantics.",
    ...
    if (packet_count, len(rows), len(by_hash), len(packet_by_capture)) != (
        2014,
        9307,
        263,
        37,
        ...
        raise ValueError(
            f"corpus reconciliation changed: {packet_count}/{len(rows)}/{len(by_hash)}/{len(packet_by_capture)}"
        )
    if (
...
def main() -> int:
    ...
    outputs = {
        "property-records.csv": csv_bytes(rows),
        "accounting.json": (
    ...
    print(
        ...
        + f" {len(outputs)} property-stream artifacts"
    )
    return 0
