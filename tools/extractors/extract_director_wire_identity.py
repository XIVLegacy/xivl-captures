...
def scan() -> tuple[list[dict], list[dict], list[dict], dict]:
                            ...
                            if len(sub) >= INNER_HEADER_LEN:
                                ...
                                if direction == "s2c" and opcode in OPCODES:
                                    if size != OPCODES[opcode]:
                                        raise ValueError(
                                            f"{capture.name} 0x{opcode:04X}: {size}"
                                        )
                                    row = {
                                        **base,
                                        ...
                                        "field_u64_0": u64(app, 0)
                                        if len(app) >= 8
                                        else "",
                                        "field_u64_8": u64(app, 8)
                                        if len(app) >= 16
                                        else "",
                                        "group_type_candidate": u32(app, 0x30)
                                        if opcode == 0x017C
                                        else "",
                                        "member_count": app[0x190]
                                        if opcode == 0x017F
                                        else (app[0x70] if opcode == 0x0183 else ""),
                                    }
                                    packets.append(row)
    ...
    types = Counter(
        p["group_type_candidate"] for p in packets if p["opcode"] == "0x017C"
    )
    owner_keys = {(r["capture"], r["owner_actor_id"]) for r in roles}
    owner_events: dict[tuple[str, int], set[str]] = {}
    for role in roles:
        owner_events.setdefault((role["capture"], role["owner_actor_id"]), set()).add(
            role["event_name"]
        )
    ...
    role_member_matches = [
        m for m in members if (m["capture"], m["actor_id"]) in owner_keys
    ]
    account = {
        "schema_version": 1,
        "packet_counts": dict(sorted(counts.items())),
        "group_type_candidate_distribution": {
            str(k): v for k, v in sorted(types.items())
        },
        "literal_30001_occurrences": types[30001],
        "literal_30001_captures": sorted(
            {p["capture"] for p in packets if p["group_type_candidate"] == 30001}
        ),
        "event_start_count": len(roles),
        "event_owner_high_nibble_distribution": {
            str(k): v
            for k, v in sorted(Counter(r["owner_high_nibble"] for r in roles).items())
        },
        "member_rows": len(members),
        "member_actor_high_nibble_distribution": {
            f"{opcode}:{nibble}": count
            for (opcode, nibble), count in sorted(
                Counter((m["opcode"], m["actor_high_nibble"]) for m in members).items()
            )
        },
        "role_member_match_rows": len(role_member_matches),
        "role_member_match_actor_ids": sorted(
            {m["actor_id"] for m in role_member_matches}
        ),
        "role_member_match_high_nibble_distribution": {
            str(k): v
            for k, v in sorted(
                Counter(m["actor_high_nibble"] for m in role_member_matches).items()
            )
        },
        "inputs": {
            "captures": [
                {"name": p.name, "sha256": sha(p)} for p in default_corpus_paths()
            ]
        },
        "boundaries": [
...
def main() -> int:
    ...
    outputs = {
        "group-packets.csv": csv_bytes(packets),
        "group-members.csv": csv_bytes(members),
    ...
    print(
        ...
        + f" {len(outputs)} director wire identity artifacts"
    )
    return 0
